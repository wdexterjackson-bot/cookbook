...
struct RecipePhotoUploadServicingTests {
    ...
    @Test func uploadReturnsAURLAndDeleteRemovesTheStoredData() async throws {
        ...
        let url = try await service.upload(imageData: data, groupID: "group-1", ownerUserID: "alice", sourceRecipeID: "recipe-1")

        #expect(url.absoluteString.contains("group-1"))
        #expect(service.uploadedData["group-1/alice_recipe-1"] == data)

        try await service.delete(groupID: "group-1", ownerUserID: "alice", sourceRecipeID: "recipe-1")

        #expect(service.uploadedData["group-1/alice_recipe-1"] == nil)
    }
    ...
    @Test func uploadThrowsWhenConfiguredToFail() async throws {
        ...
        await #expect(throws: URLError.self) {
            try await service.upload(imageData: Data(), groupID: "group-1", ownerUserID: "alice", sourceRecipeID: "recipe-1")
        }
    }
}
